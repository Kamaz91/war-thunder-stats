<script lang="ts">
  ...
  export default {
    ...
    methods: {
      ...
      async getReports() {
        let reportscount = await window.Api.reportArray.count();
        const limit = 500;

        if (!reportscount.status) {
          this.Notifier.addNotification('error', 5000, false, "Gathering error!", "Cant gather reports from database");
          return;
        }

        console.log(reportscount.request);
        for (let offset = 0; offset < reportscount.request;) {
          await window.Api.reportArray.getChunk(offset, limit).then((reports) => {
            console.log(reports);
            for (const report of reports.request) {
      ...
      async SetSettings() {
        let path = await window.Api.utility.getDataPath();
        if (path.status) {
          this.Settings.configDir = path.request;
          return;
        }
      }
    }
