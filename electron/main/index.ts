...
app.whenReady().then(createWindow).then(async (window) => {
  ...
  if (platform === 'linux') {
    appDataPath = path.join(os.homedir(), '.config', 'wt-stats');
  } else if (platform === 'win32') {
    appDataPath = path.join(app.getPath('appData'), 'wt-stats');
  } else {
    console.log('appData or .config Path not found. Running on an unsupported platform:', platform);
  }
